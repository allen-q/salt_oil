...
class UResNet(nn.Module):
    def __init__(self,pretrained=False):
        ...
        self.e5_bottleneck = nn.Conv2d(512, 192, kernel_size=1, padding=0)

    def crop_tensor(self, x, x_crop_start, x_crop_end):
        return x[:,:,x_crop_start:x_crop_end,x_crop_start:x_crop_end]
    ...
    def forward(self, x):
        ...
        d5_logit = F.upsample(self.logit_decoder(d5),
                              scale_factor=16, mode='bilinear', align_corners=False)
        d4_logit = F.upsample(self.logit_decoder(d4),
                              scale_factor=8, mode='bilinear', align_corners=False)
        d3_logit = F.upsample(self.logit_decoder(d3),
                              scale_factor=4, mode='bilinear', align_corners=False)
        d2_logit = F.upsample(self.logit_decoder(d2),
                              scale_factor=2, mode='bilinear', align_corners=False)
        d1_logit = self.logit_decoder(d1)

        logit_dec = (
            torch.stack([self.crop_tensor(d5_logit, x_crop_start, x_crop_end),
                         self.crop_tensor(d4_logit, x_crop_start, x_crop_end),
                         self.crop_tensor(d3_logit, x_crop_start, x_crop_end),
                         self.crop_tensor(d2_logit, x_crop_start, x_crop_end),
                         self.crop_tensor(d1_logit, x_crop_start, x_crop_end)])
                 .squeeze().permute(1,0,2,3)
            )
        
        img_clf = F.adaptive_avg_pool2d(self.e5_bottleneck(e5), output_size=1)
        f = torch.cat((
                d1,
        ...
        return (logit_pixel, logit_image, logit_dec)
